...
- **On Browse the categories stick too**, so switching category never means scrolling back to the top. Both bars live in one `.stickyhead` wrapper: two sticky siblings would need the second to know the first's exact height, and that height changes when the top bar wraps.
- **Category tiles sit directly under the search**, in their own white band. Browse has no hero; it goes search → categories → benefits. There is a visually hidden `<h1>` so the page still announces a heading.
- **Below 1024px the tiles become one horizontally scrolling row of chips.** A grid of six costs three rows and over 300px — tolerable when it scrolls away, not when it's pinned. The count moves beside the name instead of under it, which is what halves the height.
- **Two cues say the row swipes:** a chip cut off at the edge (the row bleeds past the gutter — don't pad that away), and a gradient fading the row into the bar at whichever edge still has chips behind it. The fade is driven by `updateTileFades()` and must stay conditional — one still lit at the end of the row promises chips that aren't there. Where a chip happens to land flush, the fade makes it read as cut anyway.
- **Below 768px only the brand row scrolls away; nav, search and categories stay.** The top bar wraps to three rows on a phone — brand, nav, search — and pinning all of it would hold 233px of an 812px screen. Pinning from the nav down holds 186px. The nav is in because the links are worth reaching from anywhere and a pinned block starting with a bare search field reads as a fragment. Done with a negative sticky top set from `--head-offset`, measured to the nav's top less the row gap so it doesn't land hard against the screen edge.
- `--stickyhead-h` and `--head-offset` are measured in `trackStickyHead()` and read by anything that has to clear the bar. **Measure on real events, not on `ResizeObserver` alone** — the tiles arrive with the catalog, so the header is ~70px shorter at first paint, and RO is silently inert in some engines. `renderTiles()` re-measures once the tiles are in.
